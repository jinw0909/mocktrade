...
async def lifespan(app: FastAPI):
    # ---------- STARTUP ----------
    logger.info("FastAPI lifespan startup - PID %s", os.getpid())
    # 여러 worker가 들어와도, scheduler 내부에서 파일 락으로 1개만 실행
    start_scheduler()

    yield

    # ---------- SHUTDOWN ----------
    logger.info("FastAPI lifespan shutdown - PID %s", os.getpid())
    # shutdown_scheduler()는 락 가진 프로세스만 의미 있게 동작
    shutdown_scheduler()
