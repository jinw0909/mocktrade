...
# from utils.connection_manager import manager
# from utils.local_redis_okx import update_position_status_per_user_okx, update_order_status_per_user_okx, update_balance_status_per_user_okx
#
...
#
#
# class CalculationOkxService(MySQLAdapter):
#
#     def __init__(self):
#         super().__init__()
...
#             self._position_redis = aioredis.from_url(
...
#             self._price_redis = aioredis.from_url(
#                 f"redis://{config.get('REDIS_HOST_OKX')}:6379/0", decode_responses=True
#             )
...
#
#     async def calculate_pnl_okx(self):
#
#         position_redis = await self.get_position_redis()
#         price_redis = await self.get_price_redis()
#
#         async for key in position_redis.scan_iter("positions_okx:*"):
#             try:
#                 _, user_id = key.split(":")
#             except ValueError:
#                 continue
...
#             await pipe.execute()
#
#     async def calculate_liq_prices_okx(self):
#         position_redis = await self.get_position_redis()
#         price_redis = await self.get_price_redis()
#
#         # ----------------------------------------------------------
...
#             return
#
#         async for pos_key in position_redis.scan_iter("positions_okx:*"):
#             try:
...
#                 continue
#
#     async def settle_orders_okx(self):
#         logger.info(f"executing settle orders at {datetime.now(timezone('Asia/Seoul'))}")
#         position_redis = await self.get_position_redis()
#         price_redis = await self.get_price_redis()
#         await self.settle_iso_liquidation_okx(position_redis)
#         await self.settle_limit_orders_okx(position_redis, price_redis)
#         await self.settle_tpsl_orders_okx(position_redis, price_redis)
#
#     async def settle_limit_orders_okx(self, position_redis, price_redis):
...
#         async for key in position_redis.scan_iter("orders_okx:*"):
#             _, user_id = key.split(":")
...
#             await update_position_status_per_user_okx(user_id, retri_id)
#             await update_order_status_per_user_okx(user_id, retri_id)
#             await update_balance_status_per_user_okx(user_id, retri_id)
#
#         return row_count
#
#     async def settle_tpsl_orders_okx(self, position_redis, price_redis):
...
#
#             cursor.execute("""
#                 SELECT `status` FROM `order_history_okx`
#                 WHERE `id` = %s
#             """, (order_id, ))
...
#
#             cursor.execute("""
#                 UPDATE position_history_okx SET `status` = 2
...
#             conn and conn.close()
#
#     async def settle_iso_liquidation_okx(self, position_redis):
...
class CalculationOkxService(MySQLAdapter):
    ...
    #             ord_key = f"orders_okx:{user_id}"
    #             avl_key = f"availables_okx:{user_id}"
    #             # liq_key = f"liq_prices_okx:{user_id}"  # 필요시 재활성화
    #
    #             balance = float(await position_redis.get(bal_key) or 0)
    #             raw_ord = await position_redis.get(ord_key) or "[]"
    #             orders = json.loads(raw_ord)
    ...
    #                     await update_position_status_per_user_okx(uid, user_id)
    #                     await update_order_status_per_user_okx(uid, user_id)
    #                     await update_balance_status_per_user_okx(uid, user_id)
    #
    #                     # (선택) 소켓 알림은 유지
    #                     await self.send_redis_signal(
    #                         retri_id=user_id,
    ...
    async def calculate_liq_prices_okx(self):
        ...
        async for pos_key in position_redis.scan_iter("positions_okx:*"):
            try:
                ...
                # ---- redis keys
                bal_key = f"balances_okx:{retri_id}"
                ord_key = f"orders_okx:{retri_id}"
                avl_key = f"availables_okx:{retri_id}"
        ...
        for mysql_uid, retri_id in liquidated_users.items():
            try:
                await update_position_status_per_user_okx(mysql_uid, retri_id)
                await update_order_status_per_user_okx(mysql_uid, retri_id)
                await update_balance_status_per_user_okx(mysql_uid, retri_id)
            except Exception:
    ...
    async def settle_limit_orders_okx(self, position_redis, price_redis):
        ...
        async for key in position_redis.scan_iter("orders_okx:*"):
            ...
            if not limit_orders:
                continue
            # logger.info(f"limit orders of user {user_id} : {limit_orders}")
            # balance_key = f"balances_okx:{user_id}"
            pos_key = f"positions_okx:{user_id}"
            try:
                # wallet_balance = float(await position_redis.get(balance_key) or 0)
                raw_pos = await position_redis.hgetall(pos_key)
