...
class MySQLAdapter:
    ...
    def _get_redis(self):
        try:
            ...
            # print(config.get('PASS'))
            # print(config.get('DBNAME'))
            connection = rd = redis.Redis(host=config.get('REDIS_HOST1'), port=6379, db=0)
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    # 쿼리에서 타이핑 오류 수정: usder_id -> user_id
                    sql = """
                          INSERT INTO position_history_okx
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = """UPDATE position_history_okx SET status = %s WHERE id = %s"""
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = f"SELECT * FROM position_history_okx where status=1 and user_id={user_no} and symbol='{symbol}';"
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = f"SELECT * FROM position_history_okx where status=1 and user_id={user_no} and symbol='{symbol}' ORDER  BY `datetime` DESC limit 1 ;"
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = f"SELECT * FROM user_okx where id={user_no}"
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = f"SELECT * FROM position_history_okx where user_id={user_no} and symbol='{symbol}' and status=1 and margin_type ='{margin_type}';"
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = f"SELECT * FROM order_history_okx where user_id={user_no} and symbol='{symbol}' and status=0 and margin_type ='{margin_type}';"
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql1 = f"SELECT * FROM position_history_okx where user_id={user_no} and status =1 and margin_type='isolated';"
                    cursor.execute(sql1)
                    result1=cursor.fetchall()
                    result1=pd.DataFrame(result1)
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = f"SELECT * FROM position_history_okx where status=1 and user_id={user_no} and side = '{side}' and symbol='{symbol}'"
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = """UPDATE position_history_okx SET pnl = %s ,close_price = %s WHERE id = %s"""
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = f"SELECT * FROM symbol_okx where symbol='{symbol}';"
    ...
    def buy_limit_order(self,user_no: int, symbol: str, margin_type: int, leverage: int,price : float, usdt=0, amount=0,tp=0,sl=0 ) :
        ...
        print('user',user_no, 'symbol',symbol,'margin_type', margin_type,'lever', leverage,'price',price,'usdt', usdt,'amount', amount, price_ch,qty_ch)
                    ...
                    if new_balance >= 0:
                        ...
                        if self.get_position_chck(user_no,symbol)==False:
                            ...
                            if position['side'].iloc[0]=='buy':
                                ...
                                new_price=self.floor_to_n_decimal(new_size/quantity,price_ch)
                                # new_margin=position['margin'].iloc[0]-new_usdt
                                # new_margin=quantity*price
                                # mar=position['size'].iloc[0]/usdt
                                # new_margin=position['margin'].iloc[0]/mar
                                tp1=position['tp'].iloc[0]
                                sl1=position['sl'].iloc[0]
                                entry_price= position['entry_price'].iloc[0]
                                new_margin1=position['margin'].iloc[0]
                                lever=position['leverage'].iloc[0]
                                print( '엔트리 프라이스',entry_price,'뉴마진',new_margin1,';;;;;;;;;;;;;;;;;;;;;;;;;;;;',new_size)

                                cc= position['size'].iloc[0]/lever
                                print('ccccc',cc,cc-position['margin'].iloc[0])
                                ...
                                if amount >0:
                                    if  quantity <0:
                                        ...
                                        if (cc-position['margin'].iloc[0])<-3:
                                            ...
                                            mar=position['size'].iloc[0]/usdt
                                            new_margin=position['margin'].iloc[0]/mar
                                            new_margin=position['margin'].iloc[0]-new_margin
                                            print('mararararararararara',mar,new_margin)
                                        else:
                                            print('요요요요기222')
                                            new_margin=position['margin'].iloc[0]-new_usdt
                                        if margin_type=='isolated':
                                        ...
                                        self.inser_oder_history(user_no, symbol, 'market', margin_type, 'buy', price, new_usdt ,self.floor_to_n_decimal(new_amount,qty_ch), leverage, 1,price,tp,sl,id)
                                        self.inser_position_history(user_no,symbol,abs(self.floor_to_n_decimal(new_size,price_ch)),abs(self.floor_to_n_decimal(quantity,qty_ch)),entry_price,liq_price,0,abs(new_margin),0,margin_type,'sell',leverage,1,tp1,sl1,0)
                                        self.update_positon(id)
                    ...
                    if new_balance >= 0:
                        ...
                        if self.get_position_chck(user_no,symbol)==False:
                            ...
                            if position['side'].iloc[0]=='sell':
                                ...
                                print('반대포지션', 'new_size',new_size,'quantity',quantity)
                                cc= position['size'].iloc[0]/lever
                                print('ccccc',cc,cc-position['margin'].iloc[0])
                                if leverage !=lever:
                                ...
                                if amount >0:
                                    ...
                                    if  quantity <0:
                                        ...
                                        # self.close_position(usder_no,id)
                                        self.inser_oder_history(user_no, symbol, 'market', margin_type, 'sell', price, new_usdt ,self.floor_to_n_decimal(new_amount,qty_ch), leverage, 1,price,tp,sl,id)
                                        self.inser_position_history(user_no,symbol,abs(self.floor_to_n_decimal(new_size,price_ch)),abs(self.floor_to_n_decimal(quantity,qty_ch)),entry_price,liq_price,0,abs(new_margin),0,margin_type,'buy',leverage,1,tp1,sl1,0)
                                        self.update_positon(id)
                                        self.update_pnl(id,new_profit1,price)
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = """UPDATE order_history_okx SET status = %s WHERE id = %s"""
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = f"SELECT * FROM order_history_okx where id={order_id};"
        ...
        try:
            ...
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = f"UPDATE position_history_okx SET {column} = %s WHERE id = %s"
                    values = (0, id)
                    cursor.execute(sql, values)
                    conn.commit()
        except Exception as e:
        ...
        try:
            if conn:
                with conn.cursor(pymysql.cursors.DictCursor) as cursor:
                    if type == 'all':
                        sql = "SELECT * FROM order_history_okx WHERE user_id = %s AND status = 0"
                        params = (user_no,)
                    elif type == 'limit':
                        sql = "SELECT * FROM order_history_okx WHERE user_id = %s AND status = 0 AND type = 'limit'"
                        params = (user_no,)
                    elif type == 'stop-limit':
                        sql = "SELECT * FROM order_history_okx WHERE user_id = %s AND status = 0 AND (type = 'tp' OR type = 'sl')"
                        params = (user_no,)
                    else:
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = """UPDATE order_history_okx SET status  = %s  where symbol =%s AND (`type` ='tp' or `type`='sl') and user_id=%s and status=0 ;"""
            ...
            try:
                ...
                if conn:
                    with conn.cursor() as cursor:
                        ...
                        sql = f"SELECT * FROM position_history_okx where id={position_id}"
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql = f"SELECT * FROM position_history_okx where status=1 and user_id={user_no} and id='{position_id}' ;"
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    ...
                    if type == 'tp':
                        sql = f"SELECT * FROM order_history_okx where (status=0 or status=3) and user_id={user_no} and symbol ='{symbol}' and type ='tp' and order_price = 0 ;"
                    elif type =='sl':
                        sql = f"SELECT * FROM order_history_okx where (status=0 or status=3) and user_id={user_no} and symbol ='{symbol}' and type ='sl' and order_price = 0 ;"
    ...
    def update_order_tp_sl_position(self,price,id,type):
        ...
        try:
            if conn:
                with conn.cursor() as cursor:
                    if type == 'tp':
                        sql = """UPDATE order_history_okx SET   tp= %s  where id=%s ;"""
                    else:
                        sql = """UPDATE order_history_okx SET   sl= %s  where id=%s ;"""
            ...
            try:
                ...
                if conn:
                    with conn.cursor() as cursor:
                        ...
                        sql = f"SELECT * FROM position_history_okx where user_id={user_no} and status=1"
    ...
    def calculate(self,symbol:str,  margin_type: int, leverage: int,side:int,price:float,  usdt:float,balance=0):
        check = MakeErrorType()
        # buy
        margin_type = 'isolated' if margin_type == 0 else 'cross'
        price_ch,qty_ch=self.get_qty(symbol)
